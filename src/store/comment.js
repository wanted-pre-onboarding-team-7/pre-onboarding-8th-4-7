...
export const addCommentAPI = (profile_url,author,content,createdAt) => {
    return async(dispatch,getState) => {
        try {
            await axios.post('http://localhost:4000/comments',{profile_url,author,content,createdAt})
            .then((res)=>dispatch(addComment(profile_url,content,author,createdAt,res.data.id)))
...
export const initComment = (data) => ({type: INIT_COMMENT,
  data
})
...
export const editCommentAPI = (id,profile_url,author, content, createdAt) => {
    return async(dispatch,getState) => {
        try {
            await axios.put(`http://localhost:4000/comments/${id}`,{profile_url,author,content,createdAt})
